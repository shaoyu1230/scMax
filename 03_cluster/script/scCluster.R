...
for (method in methods) {
  ...
  for (res in resolutions) {
    ...
    pumap <- CellDimPlot(tmp, reduction='umap', group.by='seurat_clusters', label=TRUE, label_insitu=TRUE, palcolor=pal, theme_use="theme_blank")
    ggsave(file.path(res_dir, "1_Cluster_UMAP.pdf"), pumap, width=7, height=5)
    ptsne <- CellDimPlot(tmp, reduction='tsne', group.by='seurat_clusters', label=TRUE, label_insitu=TRUE, palcolor=pal, theme_use="theme_blank")
    ...
    if (isTRUE(opt$do_cluster)) {
       cat("     - [高级选项] 正在执行全方位群体特征表征 (cluster_characterization) ...\n")
       ...
       tryCatch({
         ...
         cat("       (Cluster 高级表征执行报错失败，将跳过当前组：", conditionMessage(e), ")\n")
       })
    }
