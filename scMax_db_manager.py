...
def main():
    ...
    # 优先找 Keep，如果没有再找 All (使用通配符忽略之前或之后有无数字开头的状况)
    umap_path = find_file('**/*annotation/CellType_Keep/*CellType.UMAP-blank.png')
    if not umap_path:
        umap_path = find_file('**/*annotation/CellType_All/*CellType.UMAP-blank.png')

    # 细胞占比图
    fraction_path = find_file('**/*celltype_fraction/*CellType_Keep.in.Sample-bar.png')

    marker_table_path = find_file('**/*annotation/CellType_All/*Cluster-CellType.anno.xls')

    report_path = find_file('report_custom.html')
