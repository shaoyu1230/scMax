...
# data@meta.data[,'Sample'] <- data@meta.data[,sample_col]

########## 1_cluster_characterization-plots ################
print(paste0(Sys.time(),' - ############## start 1_cluster_characterization'))
if(length(group_col)>0){
  plot_groups <- unique(c(plot_groups,'Sample','Group'))
}else{
  plot_groups <- 'Sample'
}
print('Plot Groups:')
print(plot_groups)
cluster_plots(data,outputDir=paste0(outdir,'/1_cluster_characterization'),groups=plot_groups,palcolor=cluster_colors)

print(paste0(Sys.time(),' - ############## 1_cluster_characterization done!'))
...
print(paste0(Sys.time(),' ############## - CellType Annotation Part1 done!'))
