...
plot_refmarkers <- function(data,refmarker.file,outdir=paste0(outdir,'/2_marker_expression')){
...
cluster_de <- function(data,rdsdir,outdir='1_cluster_properties/4_Cluster.DE'){
  if(!dir.exists(outdir)){dir.create(outdir)}
...
cell_fraction_plots <- function(data,out_prefix="4_celltype_fraction/1_CellType_All.in.",celltype_col='CellType',
...
celltype_enrich <- function(derds='CellTypeDEGs.rds',orgdb='org.Hs.eg.db',organism_kegg='hsa',
                            enrichdir='5_celltype_characterization/2_CellType_Enrich'){
